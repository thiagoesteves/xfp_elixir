defmodule Xfp do
  ...
  defp pin_presence_analyse(@xFP_PRESENT) do true  end
  defp pin_presence_analyse(_)            do false end
  ...
  defp upload_xfp_static_information(state) do
    ...
    # >1 byte information
    vendor_name = read_xfp_string(state[:inst],
                                  @xFP_REG_VENDOR_NAME,
                                  @xFP_REG_VENDOR_NAME_SIZE)
    vendor_part = read_xfp_string(state[:inst],
                                  @xFP_REG_PART_NUMBER,
                                  @xFP_REG_PART_NUMBER_SIZE)
    vendor_serial = read_xfp_string(state[:inst],
                                    @xFP_REG_VENDOR_SERIAL,
                                    @xFP_REG_VENDOR_SERIAL_SIZE)
    vendor_datecode = read_xfp_string(state[:inst],
                                      @xFP_REG_DATE_CODE,
                                      @xFP_REG_DATE_CODE_SIZE)
    revision = read_xfp_string(state[:inst],
                               @xFP_REG_REVISION,
                               @xFP_REG_REVISION_SIZE)
    # For Vendor OUI, we capture the list and convert
    vendor_oui = 
      read_xfp_string(state[:inst],
                      @xFP_REG_VENDOR_OUI,
  ...
  defp read_priv(:temperature, instance) do
    temp = 
      read_xfp_string(instance, @xFP_REG_TEMPERATURE,
  ...
  defp read_priv(:tx_bias, instance) do
    tx_bias = 
      read_xfp_string(instance, @xFP_REG_TX_BIAS,
  ...
  defp read_priv(:tx_power_mw, instance) do
    tx_power = 
      read_xfp_string(instance, @xFP_REG_TX_POWER,
  ...
  defp read_priv(:rx_power_mw, instance) do
    rx_power = 
      read_xfp_string(instance, @xFP_REG_RX_POWER,
